...
class PurchaseSubscription(models.Model):
    ...
    def get_vendor(self):
        vendor_ids = self.env['account.move'].search([('move_type', '=', 'in_invoice')]).mapped('partner_id')
        return [('id', 'in', vendor_ids.ids)]
    ...
    recurring_tax_amount = fields.Float(
        compute="_compute_recurring_total",
        string="Tax",
        store=True,
        tracking=True,
    ...
    def _compute_recurring_total(self):
        ...
        for sub in self:
            ...
            sub.recurring_tax_amount = sum(
                line.price_tax for line in sub.recurring_invoice_line_ids
            )
            sub.recurring_total_amount = sum(
                line.price_total for line in sub.recurring_invoice_line_ids
            )
    ...
    def _prepare_invoice_data(self):
        ...
        return {
            ...
            "ref": self.code + ' ' + datetime.strftime(datetime.today(), '%d-%b-%Y'),
            "fiscal_position_id": fpos_id,
            "currency_id": self.currency_id and self.currency_id.id or False,
            "purchase_order_id": self.purchase_id.id or False,
            "purchase_subscription_id": self.id or False,
