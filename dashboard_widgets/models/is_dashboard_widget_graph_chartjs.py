...
class DashboardWidgetGraph(models.Model):
    ...
    def chart_get_data(self, dom, model, measure_field, groupby, show_empty_groups, action, title=False, orderby=False, orderby_default_sort_label=True, limit=False, sudo=False, query_name=False, label_regx=None):
        ...
        def _get_label_value(item):
            if color_group_field not in item:
                return "unknown", "unknown"

            group_field_without_aggregate = color_group_field.split(':')[0]
            field = self.env[model.model]._fields[group_field_without_aggregate]

            if field.type == "many2one":
                if type(item[color_group_field]) == tuple:
                    return 'many2one', self.env[field.comodel_name].browse(item[color_group_field][0]).exists()
                else:
                    return 'many2one', self.env[field.comodel_name].browse(item[color_group_field]).exists()
            if field.type == "many2many":
                return "many2many", "unknown"
            elif field.type == "boolean":
                return 'boolean', item[color_group_field]
            elif field.type.startswith("date"):
                return 'text', item[color_group_field]
                ...
                # Return display name for selection to help users set up themes without needing to know internal selection name
                return 'selection', get_string_for_field_selection_value(group_field_without_aggregate, item[color_group_field])
            elif field.type == "float":
                return 'float', item[color_group_field]
            elif field.type == "monetary":
                return 'float', item[color_group_field]
            elif field.type == "integer":
                return 'float', item[color_group_field]
            elif field.type == "reference":
                return 'reference', item[color_group_field]
            else:
                return "text", "unknown"
        ...
        if len(groupby) > 2:
            ...
            color_group_field = groupby[len(groupby)-1][2]
            ...
            for group in group2_values:
                ...
                if self.match_dataset_colors and group_data[:1]:
                    label_color = self.get_color_for_value(*_get_label_value(*group_data[:1]))
                    if label_color:
                        dataset_dict['backgroundColor'] = label_color
                        dataset_dict['borderColor'] = label_color
                        dataset_dict['pointBackgroundColor'] = label_color
                        dataset_dict['pointBorderColor'] = label_color
    ...
    def get_color_for_value(self, type, value):
        color = False
        if not value:
            color = '#e31a1c'  # red
        elif type == 'many2one':
            # if chart_color field exists and is set on record
            color_field = value._fields.get('chart_color', value._fields.get('x_chart_color'))
            if color_field:
                color = value[color_field.name] if value[color_field.name] else False
        return color
