...
class DashboardWidget(models.Model):
    ...
    tag_ids = fields.Many2many('is.dashboard.widget.tag', string="Dashboard Tags", relation="is_dashboard_widget_is_dashboard_widget_tag_rel")
    ...
    match_dataset_colors = fields.Boolean(string="Use same colours across dashboards", default=True, help="This will use the colour defined in the chart_color or x_chart_color field when available.")
    ...
    def _setup_render_dashboard_markup_error(self):
        if isinstance(self.env.context, frozendict):
            self.env.context = frozendict({**self.env.context, **{'render_dashboard_markup_errors': []}})
        else:
    ...
    def _add_render_dashboard_markup_error(self, title, error):
        if isinstance(self.env.context, frozendict):
            if not hasattr(self.env.context, 'render_dashboard_markup_errors'):
                self.env.context = frozendict({**self.env.context, **{'render_dashboard_markup_errors': []}})
            error_list = self.env.context['render_dashboard_markup_errors'] + [(title, error)]
            self.env.context = frozendict({**self.env.context, **{'render_dashboard_markup_errors': error_list}})
        else:
    ...
    def compute_dashboard_data(self):
        for rec in self:
            rec.dashboard_data = json.dumps(rec.sudo().get_dashboard_data(), default=self._dashboard_json_converter)
...
class DashboardWidget2(models.Model):
    ...
    def _compute_display_name(self):
        ...
        for rec in self:
            ...
            rec.display_name = rec.sudo()._render_content_as_template(rec.name)
