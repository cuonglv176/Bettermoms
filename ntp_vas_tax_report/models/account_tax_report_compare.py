...
class NTPAccountTaxReportCompare(models.AbstractModel):
    _inherit = 'account.report'
    _name = 'account.tax.report.compare'
    ...
    @api.model
    def _default_tax_report_populate_lines_recursively(self, options, lines, sorting_map_list, groupby_fields,
                                                       values_node, index=0, type_tax_use=None):
        ...
        for key in sorted_keys:
            ...
            report_line = self._default_tax_report_build_report_line(options, default_vals, groupby_key,
                                                                     sorting_map[key][0])
    ...
    def _redirect_to_misconfigured_tax_groups(self, company, countries):
        ...
        raise RedirectWarning(
            _('Some of your tax groups are missing information in company %s. Please complete their configuration.',
    ...
    def _get_columns_name(self, options):
        ...
        if self._is_generic_layout(options):
            ...
            columns_header.append({'name': '%s \n %s' % (_('Balance'), self.format_date(options)), 'class': 'number',
                                   'style': 'white-space: pre;'})
            columns_header.append({'name': '%s \n %s' % (_('Einvoice Balance'), self.format_date(options)), 'class': 'number',
                                   'style': 'white-space: pre;'})
            columns_header.append({'name': '%s \n %s' % (_('Difference Balance'), self.format_date(options)), 'class': 'number',
                                   'style': 'white-space: pre;'})
            if options.get('comparison') and options['comparison'].get('periods'):
                for p in options['comparison']['periods']:
    ...
    def _compute_from_amls_grids(self, options, dict_to_fill, period_number):
        ...
        """
        tables, where_clause, where_params = self._query_get(options)
        sql = """select account_tax_report_line_id, sum(balance) as balance, sum(einvoice_amount) as einvoice_amount from (
                ...
                SELECT
                       ...
                       case when account_move_line.ignore_mismatch = true then sum(COALESCE(account_move_line.balance, 0)
                             * CASE WHEN acc_tag.tax_negate THEN -1 ELSE 1 END
                             * CASE WHEN account_move_line.tax_tag_invert THEN -1 ELSE 1 END
                             )
                         else
                         (
                       case when max(ti.account_move_id) is not null then (
                       case when report_line.vas_tax_type = 'vat_amount' then
                       sum(ti.vat_amount) / (case when count(account_move_line.id) > 0
                                                   then count(account_move_line.id) else 1 end) else
                       sum(ti.total_price_before_vat) / (case when count(account_move_line.id) > 0
                                                   then count(account_move_line.id) else 1 end) end )
                       else (
                       case when report_line.vas_tax_type = 'vat_amount' then
                       sum(einvoice.total_vat_amount) / (case when count(account_move_line.id) > 0
                                                   then count(account_move_line.id) else 1 end) else
                       sum(einvoice.total_amount_without_vat) / (case when count(account_move_line.id) > 0
                                                   then count(account_move_line.id) else 1 end)  end )  end) end as einvoice_amount
                  FROM """ + tables + """
                  JOIN account_move
                  ...
                  left join account_tax_group ag
                            on acc_tag.tax_group = ag.id
                ...
                left join (select ti.account_move_id, sum(til.total_price_before_vat) as total_price_before_vat, sum(til.vat_amount) as vat_amount, til.vat_percentage
                 from tax_invoice ti inner join tax_invoice_line til on ti.id = til.tax_invoice_id 
                 where ti.state  in ('validated')
                 group by  ti.account_move_id, til.vat_percentage) ti
                    on account_move_line__move_id.id = ti.account_move_id and ti.vat_percentage =  NULLIF(regexp_replace(ag.name, '\D','','g'), '')::numeric
                left join (
                        select amner.account_move_id, sum(ne.total_vat_amount) as total_vat_amount, sum(ne.total_amount_without_vat) as total_amount_without_vat, ne.vat_percent
                         from ntp_einvoice ne
                        inner join account_move_ntp_einvoice_rel amner on ne.id = amner.ntp_einvoice_id
                        where ne.provider_einvoice_status not in ('be_replaced', 'canceled')
                        group by  amner.account_move_id, ne.vat_percent
                ) einvoice on einvoice.account_move_id = account_move_line__move_id.id and einvoice.vat_percent =  NULLIF(regexp_replace(ag.name, '\D','','g'), '')::numeric
                 WHERE """ + where_clause + """
                   AND report_line.report_id = %s
                   AND account_journal.id = account_move_line.journal_id
                 GROUP BY account_move_line.move_id,account_tax_report_line_tags_rel.account_tax_report_line_id, report_line.vas_tax_type,account_move_line.ignore_mismatch
                   ) tax_compare GROUP BY account_tax_report_line_id

            """
        params = where_params + [options['tax_report']]
        self.env.cr.execute(sql, params)
        for account_tax_report_line_id, balance, einvoice_balance in self.env.cr.fetchall():
            if account_tax_report_line_id in dict_to_fill:
                ...
                dict_to_fill[account_tax_report_line_id][0]['periods'][period_number]['einvoice_balance'] = einvoice_balance
                dict_to_fill[account_tax_report_line_id][0]['periods'][period_number]['diff_balance'] = balance - (einvoice_balance or 0)
    ...
    def compute_check(self, lines, options):
        ...
        if controls:
            lines.extend([{'id': 'section_control', 'name': _('Controls failed'), 'unfoldable': False,
                           'columns': [{'name': '',
                                        'style': 'white-space:nowrap;',
                                        'balance': ''}] * col_nber, 'level': 0, 'line_code': False,
                           'is_control': True}] + controls)
            options[
    ...
    def _build_total_line(self, report_line, parent_id, balances_by_code, formulas_dict, number_periods, deferred_line,
                          options):
    ...
    def _build_tax_section_line(self, section, parent_id, hierarchy_level):
        ...
        """
        line_id = self._get_generic_line_id('account.tax.report.line', section.id, parent_line_id=parent_id,
                                            markup='section')
    ...
    def _build_tax_grid_line(self, grid_data, parent_id, hierarchy_level, options):
        ...
        """
        columns = []
        data_key = ['balance', 'einvoice_balance', 'diff_balance']
        for i, period in enumerate(grid_data['periods']):
            ...
            for key in data_key:
                ...
                columns.append(column)
            # einvoice_column = {
            #     'name': self.format_value(einvoice_value),
            #     'style': 'white-space:nowrap;',
            #     'balance': einvoice_value,
            # }
            # columns.append(einvoice_column)
    ...
    def tax_tag_template_open_aml(self, options, params=None):
        ...
        return self.with_context(vas_type=tag_template.vas_tax_type).open_action(options, domain)
    ...
    def open_action(self, options, domain):
        ...
        ctx.update({'search_default_account': 1, 'search_default_groupby_date': 1, 'search_default_mismatch': 1, 'compute_mismatch': True})
